...
export default function Reports() {
  ...
  const filteredTimeEntries = timeEntries.filter((entry) => {
    ...
    const matchesEmployee =
      ...
      entry.userId.toString() === selectedEmployee;

    return inDateRange && matchesEmployee;
  });

  // Calculate total hours by employee
  const employeeHours = employees.reduce(
    (acc, employee) => {
      if (employee.role !== "employee") return acc;

      const entries = filteredTimeEntries.filter(
              ...
              <div>
                ...
                <Select value={reportType} onValueChange={setReportType}>
                  ...
                  <SelectContent>
                    ...
                    <SelectItem value="projectProgress">
                      Project Progress
                    </SelectItem>
          ...
          >
              ...
              <Card>
                <CardHeader className="flex flex-row items-center justify-between pb-2 pt-4 px-6">
                  <CardTitle className="text-lg">
                    Time Tracking Report: {format(range.start, "MMM d, yyyy")} -{" "}
                    ...
                    <Card>
                      <CardContent className="p-4 flex flex-col items-center">
                        ...
                        <div className="text-2xl font-bold">
                          {
                            new Set(
                              filteredTimeEntries.map((entry) => entry.userId)
                            ).size
                          }
                        </div>
                      </CardContent>
            ...
            <TabsContent value="employeePerformance" className="mt-6">
              <Card>
                <CardHeader className="flex flex-row items-center justify-between pb-2 pt-4 px-6">
                  <CardTitle className="text-lg">
                    Employee Performance: {format(range.start, "MMM d, yyyy")} -{" "}
                    {format(range.end, "MMM d, yyyy")}
                  </CardTitle>
                </CardHeader>
                <CardContent className="px-6">
                        ...
                        <div className="text-2xl font-bold">
                          {(
                            filteredTimeEntries.reduce(
                              (sum, entry) => sum + (entry.totalHours || 0),
                              0
                            ) / Math.max(1, employeeHours.length)
                          ).toFixed(1)}
                        </div>
                      </CardContent>
                    </Card>
